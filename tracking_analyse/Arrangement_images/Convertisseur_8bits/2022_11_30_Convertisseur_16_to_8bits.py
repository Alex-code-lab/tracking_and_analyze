...
for positions in list_fields_positions : 
    ...
    for i in range(0,len(frames),1) : 
        frames_8bit_skimage = img_as_ubyte(exposure.rescale_intensity(frames[i]))
        # frames_8bit_skimage=util.invert(frames_8bit_skimage)
        # plt.imshow(frames_8bit_skimage, cmap = 'gray')
        # print('max : ', frames_8bit_skimage.max())
        io.imsave(enregistrement+'/{}/{}_{}.tif'.format(position_name,position_name,i) , frames_8bit_skimage)
